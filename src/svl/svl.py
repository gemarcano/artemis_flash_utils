...
BAR_WIDTH_CHARS = 50  # Width of progress bar, ie [###### % complete
...
def get_crc16(data):
    """Compute the CRC16 of the given data.

    Keyword arguments:
    data -- FIXME it just needs to be convertible to a bytearray?
    """
    ...
    for ch in data:
        table_addr = ch ^ (crc >> 8)
        crch = (crcTable[table_addr] >> 8) ^ (crc & 0xFF)
...
def phase_setup(ser):
    ...
    logging.debug("Phase:\tSetup")

    # Handle the serial startup blip
    ser.reset_input_buffer()
    logging.debug("\tCleared startup blip")
    ...
    print(
        "Got SVL Bootloader Version: " + str(int.from_bytes(packet["data"], "big")),
    )
    logging.debug("\tSending 'enter bootloader' command")
...
def phase_bootload(ser, binpath, verbose):
    ...
    with open(binpath, mode="rb") as binfile:
        ...
        logging.debug("\thave %s bytes to send in %s frames", total_len, total_frames)

        bl_done = False
        bl_succeeded = True
        while (not bl_done) and bl_succeeded:
            # wait for indication by Artemis
            packet = wait_for_packet(ser)
            if packet["timeout"] or packet["crc"]:
                logging.debug("\tError receiving packet")
            ...
            if curr_frame <= total_frames:
                frame_data = application[
                    ((curr_frame - 1) * frame_size) : (
                        (curr_frame - 1 + 1) * frame_size
                    )
                ]
                if verbose:
                    logging.debug(
                        "\tSending frame #%s, length: %s", curr_frame, len(frame_data)
                    )
                else:
                    percent_complete = curr_frame * 100 / total_frames
                    percent_complete_in_chars = math.ceil(
                        percent_complete / 100 * BAR_WIDTH_CHARS
                    )
                    while progress_chars < percent_complete_in_chars:
                        progress_chars = progress_chars + 1
        ...
        if bl_succeeded:
            ...
            end_time = time.time()
            bps = total_len / (end_time - start_time)
            logging.debug("\n\tNominal bootload bps: %s", round(bps, 2))
        else:
            print("Upload failed")
...
def phase_serial_port_help(port):
    ...
    for dev in devices:
        if dev.device.upper() == port.upper():
            print(
                dev.device
                + " is currently open. Please close any other terminal programs that may be using "
                + dev.device
                + " and try again."
            )
            sys.exit()
    ...
    print(port.upper() + " not found but we detected the following serial ports:")
    for dev in devices:
        if "CH340" in dev.description:
            print(
                dev.description
                + ": Likely an Arduino or derivative. Try "
                + dev.device
                + "."
            )
            ...
            print(dev.description + ": Possibly an Arduino or derivative.")
        else:
...
def main():
    ...
    parser.add_argument(
        "-v", "--verbose", default=0, help="Enable verbose output", action="store_true"
    )
    ...
    parser.add_argument(
        ...
        "--timeout",
        default=0.50,
        help="Communication timeout in seconds (default 0.5)",
        type=float,
    )
    ...
    try:
        ...
        logging.debug(
            "Script version %s.%s", SCRIPT_VERSION_MAJOR, SCRIPT_VERSION_MINOR
        )

        if not os.path.exists(args.binfile):
            print(f"Bin file {args.binfile} does not exist.")
            sys.exit()
        ...
        for _ in range(num_tries):
            with serial.Serial(args.port, args.baud, timeout=args.timeout) as ser:
                # startup time for Artemis bootloader
                # (experimentally determined - 0.095 sec min delay)
                t_su = 0.15
                ...
                if entered_bootloader:
                    bl_success = phase_bootload(ser, args.binfile, args.verbose)
                    if bl_success:  # Bootload
                        # print("Bootload complete!")
                        break
                else:
                    logging.debug("Failed to enter bootload phase")
        ...
        if not entered_bootloader:
            print(
                "Target failed to enter bootload mode.",
                "Verify the right COM port is selected and that your board has the SVL bootloader.",
            )

    except serial.SerialException:
        phase_serial_port_help(args.port)
